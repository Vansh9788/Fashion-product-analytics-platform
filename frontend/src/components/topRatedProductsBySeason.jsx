...
import { fetchSeasons, fetchTopRatedProductsBySeason } from '../services/productService';
import LoadingSpinner from './loadingSpinner';
import '../css/customgrid.css';

function TopRatedProductsBySeason() {
    ...
    const fetchProducts = (season, unitsSoldValue, pageNum) => {
        setLoading(true);
        setError('');
        fetchTopRatedProductsBySeason(season, unitsSoldValue, pageNum, pageSize)
            .then(resp => {
                setTimeout(() => {
    ...
    useEffect(() => {
        if (selectedSeason) {
            fetchProducts(selectedSeason, minRating, 1);
            setPageNumber(1);
        }
    }, [selectedSeason]);
    ...
    const handlePrev = () => {
        if (pageNumber > 1) {
            fetchProducts(selectedSeason, minRating, pageNumber - 1);
            ...
            <div className="table-container">
                ...
                >
                    ...
                    <input
                        ...
                        placeholder="Customer Rating greater than"
                        value={minRating}
...
export default TopRatedProductsBySeason;
